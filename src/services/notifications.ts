 ...
 *
 * Le rappel quotidien existe déjà (optionsApp.ts). Ce service ajoute un
 * rappel CONTEXTUAL, recalculé à chaque ouverture de l'app :
...
/** Toggle (Paramètres) : rappel contextuel série. */
...
export async function planifierRappelsDuJour(): Promise<void> {
  try {
    ...
    // Série active mais rien fait aujourd'hui ? → alerte à 20h.
