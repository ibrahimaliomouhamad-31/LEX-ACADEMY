...
- **Écrans** : `exercices`, `liste_exercices`, `matieres_exos`, `chapitres_exos`, `classes_exos`, `entrainement_infini`, `qcm_eclair`, `olympiades`, `positionnement`, `maitrise`, `solveur`, `flashcards`
- **Services** : `selectionAdaptive`, `revisions`, `qaCache`, `solveur`, `flashcardsService`, `olympiades`
...
## 🧾 Dette de contenu — état réel (re-mesuré au cycle P)

`src/services/enrichirCours.ts` génère 7 sections par chapitre depuis `GABARITS`
(**10 gabarits**, un par thème détecté par `detecterTheme`). **La dette de contenu
est soldée (cycle O)** : plus aucun libellé générique, et **aucune phrase partagée
entre deux thèmes** (0 doublon mesuré sur 1027 lignes de gabarits).

| Gabarit | Objectifs | Méthodes / exemples / pièges / clés / auto-éval |
|---|---|---|
| `mathematiques`, `geometrie`, `analyse`, `probabilites`, `physique`, `chimie`, `svt` (101 lignes chacun) | ✅ spécifiques | ✅ spécifiques (formules, exemples chiffrés, pièges du BAC) |
| `trigonometrie` (87 lignes), `complexes` (89 lignes) | ✅ | ✅ **curés à la main** |
| `general` (144 lignes, fourre-tout) | ✅ | ✅ méthode, unités, gestion du temps d'épreuve |

La section précédente de ce document annonçait l'inverse (« 8 gabarits génériques,
47 lignes chacun », un gabarit `probabilites` listé comme générique) : **elle était
périmée** — d'où le re-mesure ci-dessus, plus la commande ci-dessous.

```bash
npx jest src/__tests__/nouveauxServices.test.ts -t "Gabarits de cours"
```

Ce test verrouille les 3 régressions vécues : libellés génériques (7 motifs
interdits), exemples numérotés **1 → 15 dans l'ordre**, sections non vides et sans
ligne répétée. Dette résiduelle réelle : le contenu **Firebase** des cours (champ
`theorie`) appartient à l'équipe pédagogique ; tant qu'un chapitre n'est pas rédigé,
📖 Cours affiche « ⏳ Chapitre en préparation » **et** sert les 7 sections générées
localement (aucun écran vide, aucune erreur, tout hors-ligne).

---

## 🔁 Journal des audits par morceau (cycles A → P)
...
| O | 📖 Cours : gabarits curés | **Dette de contenu soldée** : les 10 gabarits de `enrichirCours.ts` (1027 lignes) étaient génériques pour 8 d'entre eux — les cours affichaient « concept approfondi », « Etudier les concepts de… », « Piege n°1 a eviter » et **30 fois la même phrase** (détecté par un test qui refusait toute ligne répétée). Chaque thème a désormais son contenu réel, **mesuré** : `mathematiques` / `geometrie` / `analyse` / `probabilites` / `physique` / `chimie` / `svt` (101 lignes chacun : formules, exemples chiffrés, pièges du BAC, auto-évaluation), `trigonometrie` (87), `complexes` (89), `general` (144 : méthode, unités, gestion du temps d'épreuve). **0 phrase partagée entre deux gabarits** (mesuré, pas estimé) et 0 libellé générique. Le test « Gabarits de cours curés par thème » verrouille 3 régressions : 7 motifs génériques interdits, exemples numérotés 1→15 **dans l'ordre** (une permutation déterministe affichait « Exemple 15 » avant « Exemple 1 »), sections non vides et sans doublon. Anomalie de formatage corrigée au passage (`},  probabilites: {` collé sur une seule ligne, invisible pour tout `grep` de gabarit). |
| P | 🧹 Code mort + IA + guide | **Défi du jour supprimé** (écran `defi_jour` et toute sa chaîne : service, entrée d'accueil, `firestore.rules` redéployées en production — un ancien APK ne peut plus lire/écrire la collection). **Quêtes hebdomadaires supprimées** : `quetesDeLaSemaine` / `numeroSemaine` / `TOUTES_QUETES` (5 quêtes) étaient définies dans `motivation.ts` **sans aucun consommateur** (badges et ligues du même service sont, eux, bien branchés) → ~40 lignes et 1 test retirés, zéro écran touché. **LEX.AI : URL du proxy enfin configurable dans l'app** — `app.json → extra.lexaiProxyUrl` **d'abord** (voyage dans l'APK, exactement comme `extra.firebaseApiKey`), puis `.env → EXPO_PUBLIC_LEXAI_PROXY_URL`, puis l'URL Firebase historique : le **404 en HTML est vérifié en production** (Cloud Function jamais déployée, plan Blaze exigé pour `cloudbuild` + `secretmanager`, `firebase functions:list` = 0 fonction). Résolution extraite en fonction pure `resoudreUrlProxy` (**+4 tests**, nettoie les espaces / slash final d'un copier-coller wrangler) ; import d'`expo-constants` **différé** pour rester chargeable en environnement Node (aucun mock à ajouter dans les tests). **`npm run lexai:check`** (`scripts/verifierLexai.js`) : diagnostic en ligne du proxy sans lancer l'app (✅ actif / 404 non déployé / **429 = worker vivant** / 503 secret `GROQ_API_KEY` absent / 502 Groq refusé / 405 mauvaise URL), complété par `lexai:login` / `lexai:secret` / `lexai:deploy` (wrangler piloté depuis la racine). **Guide réécrit** : 14 sections / 109 questions vérifiées contre le code réel — les fonctionnalités fantômes « 🗺️ Conquête » et « 📸 Photo d'exercice » (absentes de `src/`) et le faux « 15 badges » sont retirés, 90 entrées proposent « Ouvrir cet écran ▸ » (route typée `HrefApp`, vérifiée : 0 destination inexistante). **Worker Cloudflare DÉPLOYÉ le 25/09/2026** : `https://lexai-chat.lex-academy.workers.dev` (sous-domaine `workers.dev` `lex-academy` créé via l'API, secret `GROQ_API_KEY` posé, modèle `openai/gpt-oss-120b` imposé côté serveur) → `npm run lexai:check` renvoie **HTTP 200 + `{ reponse }`** (vraie réponse IA) et l'URL est branchée dans `app.json → extra.lexaiProxyUrl` : 🧠 LEX.AI est **actif** pour la première fois (jusqu'ici, aucune IA n'était joignable en production) |
| Q | 🎯 Hub Plus + Calcul mental | **Écran `plus.tsx` désencombré et fiabilisé** : 12 cartes obsolètes/fantômes ou redondantes retirées (`positionnement`, `objectifs`, `flashcards`, `entrainement_infini`, `pieges`, `mini_jeux`, `duels`, `olympiades`, `recherche`, `calculatrice`, `mon_cahier`, `bien_etre`). Conservation de 15 entrées réelles structurées en 3 onglets clairs (📚 Apprendre 6, 🧰 Outils 6, ⚙️ Réglages 3) + **bandeau d'état 100% offline** lisant les métriques locales de l'élève (XP, ligue, série de jours, crédits, erreurs au journal, badges débloqués). Les verrous `ROUTES_STRICT_EXAMEN` protègent toujours les routes sensibles contre les deep-links en mode examen. **`calcul_mental.tsx` épuré et débloqué** : retrait de l'onglet doublon « Pis BAC » (6 pièges statiques), correction du blocage UX en fin de série (ajout du bouton interactif « 🔄 Rejouer »), typage (`QuestionCalculMental`), exhaustive-deps React respecté et réintroduction des accents français. |
